...
def demoRead(geoFile, passiveFile="", display=True, makePlots=False):
  ...
  print('Mean bifurcation angle: %.3f +/- %.3f'
        %( np.mean(properties['branchAngles']), 
          np.std(properties['branchAngles'])))
  
  tipSegs, tipPos = geometry.getTips()
  pDF = PathDistanceFinder(geometry, geometry.soma)
  tipcoords = [pDF.tipCoords(tip, pos)
            for tip, pos in zip(tipSegs, tipPos)]
  Xs = [tipcoords[i][0] for i in xrange(len(tipcoords))]
  Ys = [tipcoords[i][1] for i in xrange(len(tipcoords))]
  Zs = [tipcoords[i][2] for i in xrange(len(tipcoords))]
  npilCent = np.mean(Xs), np.mean(Ys), np.mean(Zs)
  print('Neuropil center at (%.3f, %.3f, %.3f)' 
        % (npilCent[0], npilCent[1], npilCent[2]))
  
  # get the relevant branch midpoints and endpoints for each angle...
  midcoords = [branch.coordAt(0.5) for branch in geometry.branches
                 for neighbor, (segLoc, nLoc, node)
                   in zip(branch.neighbors, branch.neighborLocations)
                   if neighbor.branchOrder > branch.branchOrder]
  endcoords = [branch.coordAt(1) for branch in geometry.branches
                 for neighbor, (segLoc, nLoc, node)
                   in zip(branch.neighbors, branch.neighborLocations)
                   if neighbor.branchOrder > branch.branchOrder]
    
  segLengths = [segment.length for segment in geometry.segments]
  segLocs = [segment.coordAt(0.5) for segment in geometry.segments]
  ...
  return properties['branchAngles'], midcoords, endcoords, npilCent, \
         segLengths, segLocs
