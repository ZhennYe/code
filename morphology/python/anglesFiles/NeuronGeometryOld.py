...
class PathDistanceFinder(object):
  ...
  def tipCoords(self, segment, pos=0.5):
    stopCoord = segment.coordAt(pos)
    return stopCoord
...
class Geometry:
  ...
  def getProperties(self, passiveFile="", display=True,
                    makePlots=False):
    def _getListStats(L, confidence = 0.05, printName="", display=True):
    ...
    medianTort, lowTort, highTort = _getListStats(tortuosities,
      printName='Tortuosity of path from Soma to tips', display=display)
  
    # measure branch tortuosities
    bTortuosities = [branch.tortuosity for branch in self.branches
                     if branch.tortuosity < float('inf')]
    medianBranchTort, lowBranchTort, highBranchTort = \
      _getListStats(bTortuosities,
    printName = 'Tortuosity of neuron branches', display=display)
    ...
    medianAngle, lowAngle, highAngle = _getListStats(branchAngles,
      printName='For neuron branches, branch angle', display=display)
    ...
    for segment in self.branches:
      ...
      if daughters:
        ...
        daughterRatios.extend(n.avgRadius / segment.avgRadius
                              for n in daughters)
    medianRallRatio, lowRallRatio, highRallRatio = _getListStats(rallRatios,
      printName='For neuron branches, Rall ratio', display=display)
    medianDaughterRatio, lowDaughterRatio, highDaughterRatio = \
      _getListStats(daughterRatios,
    ...
    properties = {
      ...
      'numBranches' : len(self.branches),
      'surfaceArea' : self.surfaceArea,
      'volume' : self.volume,
      'areaToVolumeRatio' : self.surfaceArea / self.volume,
      'tortuosity' : medianTort,
      'tortuosityList' : tortuosities,
      'branchTortuosity' : medianBranchTort,
      'branchTortuosityList' : bTortuosities,
      'branchAngles' : branchAngles,
      'rall ratio' : medianRallRatio,
      'rall ratio list' : rallRatios,
      'daughter/parent radius' : medianDaughterRatio,
      'daughter ratio list' : daughterRatios
    }
    ...
    if passiveFile:
      ...
      medianTrans, lowTrans, highTrans = _getListStats(tipsTransfer,
        printName='Coupling coefficient from soma to tips', display=display)
      properties['couplingCoefficient'] = medianTrans
      properties['couplingList'] = tipsTransfer
      model, vErr, vResid = \
      ...
      while connected:
        ...
        for neighbor, (pos, nPos, node) in zip(segment.neighbors,
          ...
          if neighbor in checkObjs:
            ...
            # there is a loop!
            if neighbor in pathSegNames and segment in pathSegNames:
...
def getBranchAngle(segment, neighbor, segLoc, nLoc, node):
  ...
  angle = (180/pi) * acos(dot / sqrt(segMag * nMag))
  return angle
