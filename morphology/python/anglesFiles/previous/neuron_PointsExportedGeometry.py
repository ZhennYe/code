...
###############################################################################
def pointsRead(geoFile, passiveFile="", display=True, makePlots=False):
  ### Read in geometry file and pre-compute various quantities
  # create geometry object
  geometry = HocGeometry(geoFile)
  
  starts, mids, ends, segNums = [], [], [], []
  neighbs, neighblocs = {'*':0}, {'*':0}
  for segment in geometry.segments:
    # xstart, ystart, zstart = segment.coordAt(0)
    starts.append([segment.coordAt(0)])
    mids.append([segment.coordAt(0.5)])
    ends.append([segment.coordAt(1)])
    segNums.append( int(segment.name.strip().split('_')[1]) )
    neighbs[segNums[-1]] = [int(neighbor.name.strip().split('_')[1])
                            for neighbor in segment.neighbors]

    
    #print([neighbor.name for neighbor in segment.neighbors])
  #print neighbs
  
   # print(segment.neighbors.name)
  
  # print geometry.getProperties(passiveFile, display=display,
  #                              makePlots=makePlots)
  # print neighb
...
if __name__ == "__main__":
  ...
  pointsRead(options.geoFile, options.passive, makePlots=options.plots)
  # display any plots
  if options.plots:
    pyplot.show()
  #exit
