...
class Geometry:
  ...
  def _connectSegments(self, _segment0, _location0, _segment1, _location1):
    ...
    if _location0 == 0.0:
      _node0 = _segment0.nodes[0]
      _neighbors0 = _segment0.neighbors0
    elif _location0 == 1.0:
      _node0 = _segment0.nodes[-1]
      _neighbors0 = _segment0.neighbors1
    else:
      raise IOError('Can only connect filaments at end')
    
    # get appropriate node on _segment1, and it's existing neighbors
    if _location1 == 0.0:
      _node1 = _segment1.nodes[0]
      _neighbors1 = _segment1.neighbors0
    elif _location1 == 1.0:
      _node1 = _segment1.nodes[-1]
      _neighbors1 = _segment1.neighbors1
    else:
      raise IOError('Can only connect filaments at end')
    
    # check to make sure the two nodes are identical
    if _node1.x != _node0.x or _node1.y != _node0.y or _node1.z != _node0.z \
       or _node1.r1 != _node0.r1 or _node1.r2 != _node0.r2 or \
       _node1.r3 != _node0.r3:
      raise IOError(\
  ...
  def _getBranch(self, _start):
    ...
    while len(_checkSeg.neighbors1) == 1:
      _oldSeg = _checkSeg
