...
from math import log, sqrt, atan, isnan, pi
...
class PathDistanceFinder(object):
  ...
  def distanceTo(self, segment, pos=0.5):
    ...
    return min(baseD + segment.length * abs(pos - startPos) for
               baseD, startPos, path in self.distances[segment])
  ...
  def _computeDistances(self):
    ...
    distances = { segment : [(0.0, startPos, segment.name + '(%.1f)' % startPos)] }
    branchOrders = { segment : 0 }
    openSegments = { segment, }
    while openSegments:
      segment = openSegments.pop()
      for currentD, startPos, segPath in distances[segment]:
        for neighbor, (connectLoc, nConnectLoc, node) \
          ...
          if neighbor not in distances:
            ...
            nPath = segPath + '->(%.1f)->' % connectLoc + neighbor.name \
                  + '(%.1f)' % nConnectLoc
            distances[neighbor] = [(pathD, nConnectLoc, nPath)]
            openSegments.add(neighbor)
            branchOrders[neighbor] = branchOrders[segment] + 1
          else:
            ...
            for ind, (loopD, loopPos, loopPath) in enumerate(loopDistances):
            ...
            if efficient:
              ...
              if insertInd is None:
                loopDistances.append((pathD, nConnectLoc, nPath))
              else:
                loopDistances.insert(insertInd, (pathD, nConnectLoc, nPath))
...
class Geometry:
  ...
  def findBranches(self):
    ...
    while openBranches:
      ...
      while commonNeighbors:
        #n1, n1Side = commonNeighbors.popitem()
  ...
  def calcForewardBranchOrder(self, doPlot=True, printAxonInfo=False):
    ...
    while openPaths:
      ...
      order, path = branchOrders[branch]
      
      for n in branch.neighbors:
        if n in path:
          # don't double back
          continue
  ...
  def calcReverseBranchOrder(self, doPlot=True):
    """
    Label each neurite tip (except the soma) with branch order 0
    Every neighboring branch is labelled with the number of nodes it must
    pass to get to a tip, provided it always moves *away* from the soma
    """
    """
    Figure out a way to make this work when there are loops!
    # calculate forward branch order, so that locally we know the direction
    # of a path relative to the soma
    self.calcForewardBranchOrder(doPlot=False, printAxonInfo=False)

    # branchOrders is a dictionary object
    #  -each branch is a key in branchOrders
    #  -the value is a tuple (branch order,
    #                         path from farthest non-soma tip to branch)
    #     (int, [list of branches, in visitation order])
    

    soma = self.soma
    openPaths = set(branch for branch in self.branches if branch.isTerminal and
                    branch != soma)
    branchOrders = {branch : (0, [branch]) for branch in openPaths}
    
    while openPaths:
      branch = openPaths.pop()
      order, path = branchOrders[branch]
      
      # check branches that are closer to the soma and not in the path
      checkBranches = [n for n in branch.neighbors if n not in path and
                       n.branchOrder < branch.branchOrder]
    """
    ...
    while openPaths:
      ...
      newOpen = [n for n in branch.neighbors if not visited[n]]
      ...
      oldOpen = [n for n in newOpen.neighbors if visited[n]]
      maxOrder = -1
      newPath = []
      for n in oldOpen:
    ...
    print('Soma branch order: %s' % str(self.somaBranch.branchOrder))
  ...
  def checkConnectivity(self, removeDisconnected=False, checkObjects=None,
      ...
      while connected:
        ...
        for neighbor, (pos, nPos, node) in zip(segment.neighbors,
          ...
          if neighbor in checkObjs:
            ...
            paths[neighbor] = paths[segment] + [neighbor.name]
          else:
            # there is a loop!
            path1 = paths[segment]
            path2 = paths[neighbor]
            ind = 0
            for name1, name2 in zip(path1, path2):
              if name1 != name2:
                ...
                break
              ind += 1
            loopPath = path1[ind:] + path2[:ind-1:-1]
            warn('Loop detected', '->'.join(loopPath))
    ...
    if removeDisconnected and len(subGraphs) > 1:
      ...
      print("Removing all but largest subgraphs")
  ...
  def _connectSegments(self, segment0, location0, segment1, location1,
    ...
    def _replaceNode(_oldNode, _replacementNode):
      # swap _oldNode out with _replacementNode in all segments that touch
      # _oldNode
      ...
      # swap node1 out with node0 all the compartments that touch node1
      ...
      for _comp in _oldNode.compartments:
        ...
        if _comp not in _replacementNode.compartments:
          _replacementNode.compartments.add(_comp)
    ...
    if implicitConnect:
      try:
        ...
        # Is this a real problem? Maybe replace assertion with returning if
        # already connected
        raise
  ...
  def mergeBranchesByOrder(self):
    ...
    # continuation)
    warn('merging branches may do weird things if there are loops')
    ...
    while openBranches:
      ...
      openBranches.update( \
        n for n in current.neighbors if not visited[n])
