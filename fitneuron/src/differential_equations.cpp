...
inline double
induced1Norm(const MatrixXd & mat, const size_t & numStates)
{
...
inline double
induced1Norm(const MatrixXd & mat)
{
  // return the induced 1-norm (maximum over columns of sum of abs value of
...
DiffEqFunctor::predictFirstStep(void)
{
  ...
  derivativesMat3.noalias() = 2 * derivativesMat2 - derivativesMat1;
  derivativesVec3.noalias() = 2 * derivativesVec2 - derivativesVec1;
                                        ...
                                        VectorXd & derivativesVec)
{
  ...
  //static const auto indices = getIndices(dFStates);
  size_t n = 0;
...
inline void
DiffEqFunctor::fillMagnus2ndOrder(const MatrixXd & dMat, const MatrixXd & dVec,
                                  const double & dT)
{
...
inline void
DiffEqFunctor::fillMagnusHalf4thOrder(const MatrixXd & dMat1,
                                      const VectorXd & dVec1,
                                      const MatrixXd & dMat2,
                                      const VectorXd & dVec2,
                                      const MatrixXd & dMat3,
...
inline void
DiffEqFunctor::fillMagnus4thOrder(const MatrixXd & dMat1,
                                  const VectorXd & dVec1,
                                  const MatrixXd & dMat2,
                                  const VectorXd & dVec2,
                                  const MatrixXd & dMat3,
...
inline void
DiffEqFunctor::fillMagnusPhony6thOrder(const MatrixXd & dMat1,
                                       const VectorXd & dVec1,
                                       const MatrixXd & dMat2,
                                       const VectorXd & dVec2,
                                       const MatrixXd & dMat3,
                              ...
                              const VectorXd & statesInitial)
{
  ...
  // subtract off the average eigenvector (aids in numerical convergence)
  ...
  const double normMagnus = induced1Norm(magnus);
  ...
  size_t numSteps, taylorDegree;
  tie(numSteps, taylorDegree) = getTruncatedParameters(normMagnus);

  #ifdef LOW_ACCURACY_MATRIX_EXP
    const double stopTol = 1.0e-2 * maxErr;
  #else
    const double stopTol = 1.1e-16;
  #endif
  #ifdef SUB_MEAN_EIGEN
    const double avgFactor = exp(meanEigenvalue / (double)numSteps);
    const double magnusBR = -meanEigenvalue;
    double ihFact = 1.0;
  #endif
  ...
  for(size_t step = 0; step < numSteps; ++step) {
    ...
    if(taylorDegree >=1) {
      ...
      #ifdef SUB_MEAN_EIGEN
        double ihTerm
          = ihFact * magnusBR / (double)(numSteps);
        ihFact += ihTerm;
                              ...
                              const VectorXd & statesInitial)
{
  ...
  for(size_t step = 0; step < numSteps; ++step) {
    ...
    taylorTerm = statesFinal;  // first term is multiplied by identity
    double augTerm = augV;
    double c1 = max(taylorTerm.lpNorm<Infinity>(), abs(augTerm)); // norm of term
    
    for(size_t term = 1; term <= taylorDegree; ++term) {
      // update working states to compute each term of the truncated Taylor
      //   series
      const double factorialFact = (double)(numSteps * term);
      taylorTerm = ( magnus.topLeftCorner(numStates, numStates) * taylorTerm
                   + augTerm * magnus.rightCols<1>().head(numStates)).eval()
                     / factorialFact;
      augTerm *= magnusBR / factorialFact;

      const double c2 = taylorTerm.lpNorm<Infinity>();  // norm of last term
      
      // update statesFinal
      statesFinal += taylorTerm;
      augV += augTerm;
      
      // if the norm has gotten too small, truncate the series early
      if(c1 + c2 < stopTol * std::max(statesFinal.lpNorm<Infinity>(), 1.0e-9)){
        break;
      }
      c1 = c2;  // book-keeping on norms
    }
    
    // scale statesFinal by the effect of the average eigenvalue
    statesFinal *= avgFactor;
    augV *= avgFactor;
  }
...
DiffEqFunctor::getTruncatedParameters(const double & normMagnus)
{
  ...
  // Ensure theta has enough storage space
  theta.resize(mMax);
  ...
  // Fill out theta vector
  #ifdef LOW_ACCURACY_MATRIX_EXP
    const double logTol = log2(maxErr);
    if(logTol <= -53) {
      // requesting better or equal to double precision, use doubleVec
      ...
      // interpolate between doubleVec and halfVec
      const double interpDouble = (logTol + 10.0) / (10.0 - 53.0);
    ...
    //cout << "logTol = " << logTol << ", theta.back() = " << theta.back() << '\n';
    //cout.flush();
  #else
    (void)halfVec;
    double const* dVecPtr = doubleVec;
  ...
  if( normMagnus <= 4.0 * theta.back() * (double)(pMax * (pMax + 3))
                    / (double)mMax ) {
    // If the norm is small enough, can compute numSteps and taylorDegree using
    // a simpler algorithm
    size_t m = 1;
    double bestCost = Inf;
    for(const double & theta_m : theta) {
      const double steps_m = ceil(normMagnus / theta_m);
    ...
    for(size_t p = 2; p <= pMax; ++p) {
      ...
      const double mMin = p * (p-1) - 1;
      auto thetaItr = theta.cbegin() + (mMin - 1);
      for(size_t m = mMin; m <= mMax; ++m) {
        const double steps_m = ceil(alpha_p / *thetaItr);
        const double cost_m = steps_m * (double)m;
        if(cost_m < bestCost) {
          bestCost = cost_m;
          numSteps = (size_t) steps_m;
